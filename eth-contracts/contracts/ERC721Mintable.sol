...
contract Ownable {
    ...
    event ownerShipTransfered(address old_owener,address new_owener);
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        _owner = newOwner;
        emit ownerShipTransfered(old_owner,newOwner);
    }
}
...
contract Pausable is Ownable {
    ...
    function set_Pause(bool mode)
    public
    onlyOwner
    {
        require(_paused!=mode, 'mode is set already');
        _paused = mode;
        if (mode)
        {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function balanceOf(address owner) public view returns (uint256) {
        ...
        return uint256(_ownedTokensCount[owner].current());
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require( to != ownerOf(tokenId),"address can't be the same as owner of the token");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender==ownerOf(tokenId),"sender is not the owner");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId)==from,"only owner of the token can transfer");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function set_tokenURI(uint256 tokenId) internal
    {
        require(_exists(tokenId),"tokenId doesn't exist");
...
contract CustomERC721Token is ERC721Metadata ("MyProperty","MP","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
    
    function mint(address to, uint256 tokenId) public onlyOwner returns(bool)
    {
        super._mint(to, tokenId);
        set_tokenURI(tokenId);
        return true;
    }
